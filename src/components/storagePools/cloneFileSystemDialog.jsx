...
function CloneFileSystemDialog({ filesystem, pool, onRefresh }) {
    ...
    const handleClone = async () => {
        ...
        setCloning(true);
        try {
            await ZfsApi.cloneFileSystem(filesystem.name, cloneName.trim());
            Dialogs.close();
            onRefresh();
        } catch (exc) {
            setError({
                dialogError: `Failed to clone file system ${filesystem.name}`,
    ...
    return (
        <Modal position="top" variant="small" isOpen onClose={Dialogs.close}>
            <ModalHeader title={`Clone File System ${filesystem.name}`} />
            <ModalBody>
                <Form isHorizontal>
                    {error.dialogError && (
                        <ModalError
                    ...
                    <FormGroup label="Clone Name" fieldId="clone-name">
                        <TextInput
                            id="clone-name"
...
export default CloneFileSystemDialog;
